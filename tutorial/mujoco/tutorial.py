...
# Dependencies are installed in ./venv; this cell should not install Colab packages.
import itertools
import os
import platform
import shutil
...
if shutil.which("ffmpeg") is None:
  print("ffmpeg is not on PATH. Install it with: brew install ffmpeg")

np.set_printoptions(precision=3, suppress=True, linewidth=100)
clear_output()
print(f"Local MuJoCo setup OK: mujoco {mujoco.__version__}")
...
humanoid_xml_path = PROJECT_ROOT / ".cache" / "mujoco" / "humanoid.xml"
humanoid_xml_path.parent.mkdir(parents=True, exist_ok=True)
if not humanoid_xml_path.exists():
  print('Downloading MuJoCo humanoid XML description from GitHub:')
  urlretrieve(
    "https://raw.githubusercontent.com/google-deepmind/mujoco/3.6.0/model/humanoid/humanoid.xml",
    humanoid_xml_path,
  )

xml = humanoid_xml_path.read_text(encoding="utf-8")
