...
def pd_controller(model, data, q_des, qdot_des, qddot_des, joint_ids):  # qpos_ids, joints_id
    dof_ids = dof_ids_from_joints(model, joint_ids)
    qpos_ids = model.jnt_qposadr[joint_ids]
    kp = model.actuator_gainprm[dof_ids, 0]
    kd = 60

    qacc_des = qddot_des + kp * (q_des - data.qpos[qpos_ids]) + kd * (qdot_des - data.qvel[dof_ids])
