...
from sim.model.motion.trajectory import interpolate_pose
...
from sim_with_mujoco.utils.mj import actuator_ids_from_joints, joint_ids_from_names
        ...
        while not glfw.window_should_close(env.viewer.window):  # 렌더링 루프 (프레임 단위)
            for _ in range(sim_steps_per_frame):  # 시뮬레이션 루프
                ...
                if now - last_poll_time >= poll_interval:  # poll 주기 설정
                    last_poll_time = now
                    polled_target, _ = env.viewer.poll_target()  # 매 프레임마다 입력 처리
