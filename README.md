...
MyJoCo(feat. MuJoCo)는 로봇 모델의 다른 제어 방식을 이해하고 실험할 수 있는 MuJoCo-based 로봇 시뮬레이터 구축 프로젝트입니다. MJCF을 제외한 MyJoCo 코드는 로보틱스 기초를 다지는 목적으로 from scratch로 제작되었습니다. 현재까지 ROBOTIS 계열, Dexjoco 계열 두 로봇을 대상으로 Teleoperation task를 수행했습니다.
...
| --- | --- | --- | --- | --- | --- |
| dexjoco_teleop.py | DexJoCo Panda-Allegro 모델을 로드하고 q_des/ref_data 기준으로 arm target을 갱신 | panel target을 allegro_palm pose target으로 직접 사용, grasp slider는 open/close alpha로 매핑 | Panda arm은 differential IK + computed torque 결과를 motor ctrl에 입력, Allegro hand는 position actuator qpos target 입력 | mj_step 사용, XML에 명시된 hand-object 접촉 friction 반영 |  Panda-Allegro dynamic motor demo |
...
| Dynamic simulator(dexjoco_teleop.py) | IK 결과를 actuator ctrl에 넣고 mj_step으로 mujoco dynamics 진행 |
...
```text
sim_with_mujoco/
  dexjoco_teleop.py            differential IK + computed torque Panda-Allegro entry
  environment/
...
- DexJoCo hand grasp는 tactile feedback, contact-aware grasp planner, force closure optimization 없이 position actuator target과 MuJoCo contact solver에 의존합니다. 작은 물체를 안정적으로 잡으려면 finger force/impedance control 또는 독립적으로 다뤄지는 제어기를 지휘할 모듈이 추가로 필요합니다.
